...
/**
 * @package DB
 * This class provides a wrapper for Database interaction. Currently uses mysqli package.
 * Several wrapper methods are provided that insert rows or return results from the database.
 * The DB information needs to be provided at initialization time (via the constructor).
 * 
 * PHP Version 5.2.4
 */
class DBConnection
{
	...
	public function __construct($host, $username, $password, $dbname, $charset )
	{
		...
		{
			throw new CustomException('EXC_DB_ERROR', 'Database connection failed: ' 
						. mysqli_connect_error());
		}

		if (!mysqli_select_db($this->conn, $dbname))
		{
			throw new CustomException('EXC_DB_ERROR', 'Failed to open $dbname database: ' 
						. mysqli_connect_error());
		}
	...
	{			
		...
		{
			throw new CustomException('EXC_DB_ERROR', 'Failed to commit transaction: ' 
						. mysqli_connect_error());
		}
	...
	public function abortTransaction()
	{
		...
		if (!mysqli_rollback($this->conn))
		{
			...
			Master::getLogManager()->log(ERROR, "Failed to rollback transaction: %s", mysqli_connect_error());
		}
	...
	private function setAutoCommitStatus($flag)
	{
		...
		{
			throw new CustomException('EXC_DB_ERROR', 'Failed to set Auto Commit: ' 
						. mysqli_connect_error());
		}
	...
	public function singleResultQuery($querystr)
	{
		...
		if (count($resultset) > 1)
			Master::getLogManager()->log(WARN, MOD_DB, "More than one column returned. Only first column returned - ignoring others.");
		...
		Master::getLogManager()->log(DEBUG, MOD_DB, "Failed to retrieve any value.");
		return FALSE;
	}
	...
	public function singleColumnQuery($querystr)
	{
		...
		if (count($resultset[0]) > 1)
			Master::getLogManager()->log(DEBUG, MOD_DB, "More than one column returned. Only first column returned - ignoring others.");
	...
	{		
			...
			if (FALSE == $fields)
			{
				throw new CustomException('EXC_DB_ERROR', "Error retrieving field information for $querystr: " 
	...
	public function insertAndReturnId($insertSQL)
	{
		...
		$effectedRows =  mysqli_insert_id($this->conn);
		
		if (!$effectedRows) {			
			throw new CustomException('EXC_DB_ERROR', "No rows inserted", false, false);
		} else {
			return $effectedRows;
		}
	}
	...
	/**
	 * Should use if the insert was already performed separately. Used to return the auto increment ID for the table
	 * after insert.
	 * @return int
	 */
	...
	{	
		...
		{
			throw new CustomException('EXC_DB_ERROR', "Failed to insert rows: " 
						. mysqli_error($this->conn));
		}
	 ...
	 * @return int						The last_insert_id() value.
	 */
	public function insertSingleRowAndReturnId($table, $columnnames, $rowvals)
	{
		$rowvals	=	array(0 => $rowvals);	//	make it two dimensional array.		
		$insertSQL = $this->buildMultipleInsertSQL($table, $columnnames, $rowvals);
	 ...
	 * @param array $types			a simple array of column types (in the same order). "string"/"datetime"/"int" are currently supported.
	 * @return mixed				FALSE if failure, number of rows (1) inserted if success.
	 * @return int					number of rows inserted -- careful with this: the number changes depending
	...
	public function updateTable($table, $columnnames, $rowvals, $condition)
	{
		...
		if ($condition == "")
		{
			throw new CustomException('EXC_DB_ERROR', "Cowardly refusing to update without a WHERE CLAUSE!");
		}
	...
	public function deleteTable($table, $condition)
	{
		...
		if ($condition == "")
		{
			throw new CustomException('EXC_DB_ERROR', "Cowardly refusing to DELETE without a WHERE CLAUSE!");
		}
	...
	public function queryDB($querystr, $fakeFlag = false)
	{
		if (empty($querystr))
		{
			throw new CustomException('EXC_DB_ERROR', "Attempt to execute an empty Query.");
		}
		
		Master::getLogManager()->log(DEBUG, MOD_DB, "Query: $querystr");
	...
	public function buildUpdateSQL($table, $columnnames, $rowvals, $condition, $ignoreFlag = FALSE)
	{
		if(count($columnnames) != count($rowvals))
		{
			...
			throw new CustomException('EXC_DB_ERROR', "Column count ($numCols) doesnot match with values count ($numVals)");
		}
	...
	*/
	 ...
	 * Used if the update/insert query is directly executed by the caller (using queryDB) and the caller
	 * needs the number of affected rows.
	 *
	 * @return int
	 */
	...
	/**
	 * This method returns the last auto increment value (from the last insert statement).
	 * This is used when we independently insert a row using the insertSingleRow method and
	 * now want to retrieve the auto_increment value that would have been inserted.
	 *
	 * @return int	The auto_increment value that is the result of the previous insert statement.
	 */
	...
	/**
	 * This returns the number of rows that would have been returned by the previous select statement
	 * if there were no LIMIT clause. Very useful in pagination.
	 * @return int
	 */
	public function countFoundRows()
	{
		return $this->singleResultQuery("SELECT FOUND_ROWS()");
	}
	...
	private function checkTranStatus()
	{
		...
		if ($this->autocommit == TRUE)
		{
			Master::getLogManager()->log(WARNING, MOD_DB, "*******An auto-commit operation is being performed.*********");
		}
	}
	...
	public function parameterizedQueryWithSingleFileUpload($querystr, $filePath)
	{
		if (empty($querystr)) {
			throw new CustomException('EXC_DB_ERROR', "Attempt to execute an empty Query.");
		}
		...
		if (empty($filePath)) {
			throw new CustomException('EXC_DB_ERROR', "No upload file path.");
		}
		
		$fileContent = file_get_contents($filePath);
		
		if($fileContent == FALSE){
			throw new CustomException('EXC_DB_ERROR', "Invalid file path.");
		}
        ...
        $status = $stmt->execute();
		Master::getLogManager()->log(DEBUG, MOD_INVOICE, $status);

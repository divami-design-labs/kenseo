			...
			if(isset($data->filesToUpload)) {
				...
				if(isset($data->artefact_id)) {
					...
					for($f=0; $f<$filesCount; $f++) {
						$rowVals = array($data->project_id, $FILES['name'][$f], $FILES['name'][$f], $docType);
				...
				for($f=0; $f<$filesCount; $f++) {
					$targetPaths[] = $this->uploadFile(array(
						...
						'name'=> $FILES['name'][$f]
					), $data->project_id, $org_id, $artIds[$f], $masked_artefact_versions[$f]);
				}
				...
				for($f=0; $f<$filesCount; $f++) {
					...
					$activityColumnNames = array("project_id", "logged_by", "logged_time", "performed_on", "activity_type", "performed_on_id");
					$activityRowValues = array($data->project_id, $userId, date("Y-m-d H:i:s"), 'A', 'N', $artIds[$f]);
					$db->insertSingleRow(TABLE_PROJECT_ACTIVITY, $activityColumnNames, $activityRowValues);
					...
					$notificationRowValues = array($userId, $FILES['name'][$f], $data->project_id, $userId, date("Y-m-d H:i:s"), 'S', $artVerId, 'U');

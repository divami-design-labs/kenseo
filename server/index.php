 ...
 * 1. First check the session for a valid SID - We will use (set) NestorSID to indicate we have an active session (same as Session ID)
 * 2. If we have NestorSID, check in the DB to see if this SID is still valid. If it is, take the Google auth tokens etc. and the user
...
try
{
	...
	if (isset($_REQUEST['logout'])) {
		...
		if ($token) {
			...
			if (!$userId) {
				// This user is not authorized to access Nestor. Redirect to a descriptive page that has a Google Logout button.
			...
			util_redirectToURL($AppGlobal['googleauth'][$project]['uiURL']);
		} else {
		...
		Master::getLogManager()->log(DEBUG, MOD_MAIN, "se cookie ");
		setcookie("DivamiNestorUserID", $userObj->id, 0, "/");
